...
JspMyAdminApp.controller('ServerDatabasesController', ['$rootScope', '$scope', '$state', 'AB0Resource', 'AB1Resource', function ($rootScope, $scope, $state, AB0Resource, AB1Resource) {
    ...
    $scope.checkOne = function (database) {
        ...
        $scope.allCheck = $scope.databases.length > 0 && $scope.databases.length === $scope.page_data.database_list.length;
    };
    $scope.dropDatabases = function () {
        AB1Resource.save({}, {token: JspMyAdminContext.token, databases: $scope.databases}, function (response) {
    ...
    $scope.goToDatabase = function (database) {
        JspMyAdminContext.setDatabase(database);
        $state.go('database_tables', {request_db: EncryptionHelper.encode(database)});
    };
    $scope.loadDatabases('');
}]);