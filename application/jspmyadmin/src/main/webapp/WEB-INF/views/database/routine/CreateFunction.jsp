...
<body>
					...
					<form action="#" accept-charset="utf-8" method="post"
						...
						<div class="group">
							<div class="group-widget group-header">
								<m:print key="lbl.function_structure" />
							</div>
							<div class="group-widget group-content">
								<div
									...
									<div style="display: block;">
										<div class="form-input" style="width: 300px;">
											<label><m:print key="lbl.function_name" /></label> <input
												type="text" name="name" id="procedure-name"
												class="form-control" value="${requestScope.command.name}">
										</div>
									...
									<div>
										<div class="form-input" style="width: 300px;">
											<label><m:print key="lbl.return_type" /></label> <select
												name="return_type" class="form-control">
												<jma:notEmpty name="#data_types_map" scope="command">
													<jma:forLoop items="#data_types_map" name="datatypeVal"
														key="datatypeKey" scope="command">
														<optgroup label="${datatypeKey}">
															<jma:forLoop items="#datatypeVal" name="datatype"
																scope="page">
										...
										<div class="form-input">
											<label><m:print key="lbl.length" /></label> <input
												type="text" name="return_length" class="form-control">
										</div>
									</div>
									<div class="form-input" style="display: block; width: 100%;">
										<label><m:print key="lbl.function_body" /></label> <input
											type="hidden" name="body" id="definition">
										<textarea rows="15" cols="100" style="width: 100%;"
											id="definition-area">RETURN ;</textarea>
									</div>
								</div>
	...
	<m:store name="msg_function_body_blank" key="msg.function_body_blank" />
	<script type="text/javascript">
		$(function() {
			...
			$('#btn-go').click(function() {
				if ($('#procedure-name').val().trim() == '') {
					$('#sidebar-error-msg').text('${msg_function_name_blank}');
					$('#sidebar-error-dialog').show();
					return;
				}
				if (codeMirrorDefinition.getValue().trim() == '') {
					$('#sidebar-error-msg').text('${msg_function_body_blank}');
					$('#sidebar-error-dialog').show();
					return;
				}
				showWaiting();
				...
				$.ajax({
					url : Server.root + '/database_create_function_post.text',
					method : 'POST',
					data : $('#create-function-form').serialize(),
