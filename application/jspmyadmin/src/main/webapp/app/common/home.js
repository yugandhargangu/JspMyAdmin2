...
JspMyAdminApp.controller('HomeController', ['$rootScope','$scope', 'AA0Resource', 'AA1Resource', 'AA2Resource', 'AA3Resource',
        function($rootScope, $scope, AA0Resource, AA1Resource, AA2Resource, AA3Resource) {
	...
	$scope.saveCollation = function() {
        AA1Resource.save({token:token, collation:$scope.input_data.collation}, function(response){
            if(response.status === 0){
                location.reload();
            }
        });
	...
	$scope.saveLocale = function() {
        AA2Resource.save({token:token, language:$scope.input_data.language}, function(response){
            location.reload();
        });
    };
    $scope.saveFontSize = function() {
        AA3Resource.save({token:token, fontsize:$scope.input_data.fontsize}, function(response){
            location.reload();
        });
    };
} ]);