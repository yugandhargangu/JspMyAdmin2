...
public class SideBarLogic extends AbstractLogic {
	...
	public String callColumn(String token) throws Exception {
		...
		try {
			...
			jsonObject = new JSONObject(jsonObject.getString(FrameworkConstants.DATABASE));
			apiConnection = getConnection(jsonObject.getString(FrameworkConstants.DATABASE));
			statement = apiConnection.preparedStatementSelect("SHOW COLUMNS FROM " + table);
			resultSet = statement.executeQuery();
			jsonArray = new JSONArray();
			while (resultSet.next()) {
				result = resultSet.getString(1);
	...
	public String callEvent(String token) throws Exception {
		...
		try {
			...
			jsonObject = new JSONObject(token);
			apiConnection = getConnection(jsonObject.getString(FrameworkConstants.DATABASE));
			statement = apiConnection.preparedStatementSelect("SHOW EVENTS");
			resultSet = statement.executeQuery();
			jsonArray = new JSONArray();
			while (resultSet.next()) {
	...
	public String callRoutine(String token) throws Exception {
		...
		try {
			...
			jsonObject = new JSONObject(token);
			apiConnection = getConnection(jsonObject.getString(FrameworkConstants.DATABASE));
			statement = apiConnection.preparedStatementSelect("SHOW PROCEDURE STATUS WHERE DB LIKE ?");
	...
	public String callFunction(String token) throws Exception {
		...
		try {
			...
			jsonObject = new JSONObject(token);
			apiConnection = getConnection(jsonObject.getString(FrameworkConstants.DATABASE));
			statement = apiConnection.preparedStatementSelect("SHOW FUNCTION STATUS WHERE DB LIKE ?");
			statement.setString(1, token);
			resultSet = statement.executeQuery();
			jsonArray = new JSONArray();
			while (resultSet.next()) {
	...
	public String callTrigger(String token) throws Exception {
		...
		try {
			...
			jsonObject = new JSONObject(token);
			apiConnection = getConnection(jsonObject.getString(FrameworkConstants.DATABASE));
			statement = apiConnection.preparedStatementSelect("SHOW TRIGGERS");
			resultSet = statement.executeQuery();
			jsonArray = new JSONArray();
			while (resultSet.next()) {
				result = resultSet.getString(1);
