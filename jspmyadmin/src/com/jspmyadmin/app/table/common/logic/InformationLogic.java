	...
	public void fillBean(Bean bean) throws ClassNotFoundException, SQLException {
		...
		try {
			...
			statement = apiConnection
					.preparedStatementSelect("SHOW CREATE TABLE `" + _table + FrameworkConstants.SYMBOL_TEN);
			resultSet = statement.executeQuery();
			if (resultSet.next()) {
			...
			close(resultSet);
			close(statement);
			statement = apiConnection.preparedStatementSelect("SELECT * FROM `" + _table + "` WHERE TRUE IS FALSE");
			resultSet = statement.executeQuery();
			ResultSetMetaData metaData = resultSet.getMetaData();
