...
<body>
			...
			<div id="main-body">
				<div style="padding: 0.2em 2em;">
					...
					<form action="#" method="post" accept-charset="utf-8"
						...
						<div class="group">
							<div class="group-widget group-header">
								<m:print key="lbl.procedure_list" />
							</div>
							<div class="group-widget group-content">
								<table class="tbl" id="procedure-list">
									<thead>
										<tr>
											...
											<th><m:print key="lbl.security_type" /></th>
									...
									<tbody>
										...
										<jma:empty name="#routine_info_list" scope="command">
											<tr class="even">
												<td colspan="8"><m:print key="msg.no_procedure_found" /></td>
											</tr>
										</jma:empty>
									</tbody>
								</table>
	...
	<m:store name="msg_drop_procedure_alert" key="msg.drop_procedure_alert" />
	<m:store name="msg_select_least_one_procedure"
		key="msg.select_least_one_procedure" />
	<m:store name="msg_procedure_name_blank" key="msg.procedure_name_blank" />
	<script type="text/javascript">
		$("#header-menu li:nth-child(3)").addClass('active');
		applyEvenOdd('#procedure-list');
		// animation time
		var waitTime = 500;
		// type of operation
		var GoType = {
		...
		var GoText = {
			drop : "${msg_drop_procedure_alert}",
			select : "${msg_select_least_one_procedure}"
		};
		// action for each operation
		var GoAction = {
			drop : "/database_procedure_drop.html",
		};
		// to specify action
		var action = '#';
		...
		$(function() {
			$('#btn-go').click(function() {
				if ($('#procedure-create-name').val().trim() == '') {
					$('#error-content').text('${msg_procedure_name_blank}');
					$('#error-dialog').show();
					return;
				}
			...
			$('#yes_btn').click(function() {
				showWaiting();
				$('#procedure-list-form').attr('action', Server.root + action);
				$('#procedure-list-form').submit();
			});
	...
	<script type="text/javascript">
		...
		$(function() {
			$('#btn-show-create').click(function() {
				...
				var url = Server.root + '/database_procedure_show_create.text';
				$.ajax({
					url : url,
					data : $('#procedure-list-form').serialize(),
					method : 'POST',
	...
	<script type="text/javascript">
		...
		$(function() {
			$('#btn-execute').click(function() {
				...
				$('#sql-form').find('input[name="edit_type"]').val('3');
	...
	<script type="text/javascript">
		$(function() {
			$('#btn-edit').click(function() {
				...
				$('#sql-form').find('input[name="edit_type"]').val('2');
				$('#sql-form').find('input[name="edit_name"]').val($('input[name="routines"]:checked:first').val());
				$('#sql-form').submit();
			});
		});
