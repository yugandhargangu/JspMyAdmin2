	...
	public File exportFile(Bean bean) throws ClassNotFoundException, SQLException, IOException {
		...
		try {
			...
			for (int j = 0; j < tableList.size(); j++) {
				...
				statement = apiConnection.getStmtSelect("SHOW CREATE TABLE `" + table + FrameworkConstants.SYMBOL_TEN);
				resultSet = statement.executeQuery();
				if (resultSet.next()) {
				...
				if (exportData) {
					...
					statement = apiConnection.getStmtSelect("SELECT * FROM `" + table + FrameworkConstants.SYMBOL_TEN);
					resultSet = statement.executeQuery();
			...
			if (FrameworkConstants.ONE.equals(exportBean.getInclude_procedures())) {
				...
				if (procedureList.size() > 0) {
					...
					while (iterator.hasNext()) {
						...
						statement = apiConnection
								.getStmtSelect("SHOW CREATE PROCEDURE `" + procedure + FrameworkConstants.SYMBOL_TEN);
						resultSet = statement.executeQuery();
						if (resultSet.next()) {
			...
			if (FrameworkConstants.ONE.equals(exportBean.getInclude_functions())) {
				...
				if (functionList.size() > 0) {
					...
					while (iterator.hasNext()) {
						...
						statement = apiConnection
								.getStmtSelect("SHOW CREATE FUNCTION `" + function + FrameworkConstants.SYMBOL_TEN);
						resultSet = statement.executeQuery();
						if (resultSet.next()) {
							bufferedWriter.write(resultSet.getString(3));
						}
			...
			if (FrameworkConstants.ONE.equals(exportBean.getInclude_triggers())) {
				...
				if (triggerList.size() > 0) {
					...
					while (iterator.hasNext()) {
						...
						statement = apiConnection
								.getStmtSelect("SHOW CREATE TRIGGER `" + trigger + FrameworkConstants.SYMBOL_TEN);
						resultSet = statement.executeQuery();
						if (resultSet.next()) {
							bufferedWriter.write(resultSet.getString(3));
						}
